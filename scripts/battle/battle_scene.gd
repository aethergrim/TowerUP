...
func _connect_signals() -> void:
    if lasso.has_signal("loot_collected"):
        lasso.connect("loot_collected", Callable(self, "_on_lasso_loot_collected"))
    if loot_system.has_signal("loot_claimed"):
        loot_system.connect("loot_claimed", Callable(self, "_on_loot_claimed"))
    if crafting_system.has_signal("ammo_generated"):
        crafting_system.connect("ammo_generated", Callable(self, "_on_ammo_generated"))
    if crafting_system.has_signal("cooling_requested"):
        crafting_system.connect("cooling_requested", Callable(self, "_on_cooling_requested"))
    if enemy_spawner.has_signal("enemy_defeated"):
        enemy_spawner.connect("enemy_defeated", Callable(self, "_on_enemy_defeated"))
    if enemy_spawner.has_signal("wave_cleared"):
        enemy_spawner.connect("wave_cleared", Callable(self, "_on_wave_cleared"))
...
func _update_status_text() -> void:
    ...
    if tower:
        ammo_value = tower.get("ammo") if tower.has_variable("ammo") else 0
        heat_value = tower.get("heat") if tower.has_variable("heat") else 0.0
        max_heat = tower.get("max_heat") if tower.has_variable("max_heat") else 0.0
    var status := "Wave: %d\nAmmo: %d\nHeat: %.1f / %.1f" % [
        wave_index,
        ammo_value,
        heat_value,
